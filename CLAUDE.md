...
You are working on **Strudel**, a fork of the [Strudel](https://strudel.cc) live-coding
music environment (upstream: `github.com/tidalcycles/strudel`) under Nick's `idea/`
folder, at `idea/_workingArea/strudel/strudel`. Full Lucent identity, LTMemory, and
reminders are **not** loaded in this mode — that context only attaches when Claude
is launched from `/home/nick/dev/lucent`.
...
`claude`, and just start talking music** — no other setup needed if 4321 is
already running.
...
*both files are kept in sync by `scripts/new_project.py`.*
